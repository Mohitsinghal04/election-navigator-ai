...
def test_chat_valid_en():
    response = client.post("/api/chat", json={"session_id": "s1", "message": "hello", "language": "en"})
    assert response.status_code == 200
    assert "response" in response.json()

@pytest.mark.parametrize("lang", ["hi", "ta", "te", "mr", "bn"])
def test_chat_localization(lang):
    response = client.post("/api/chat", json={"session_id": "s1", "message": "hello", "language": lang})
    assert response.status_code == 200
...
def test_timeline_card_trigger():
    response = client.post("/api/chat", json={"session_id": "s2", "message": "Show me the election timeline", "language": "en"})
    data = response.json()
    assert "timeline_event" in data
    assert data["timeline_event"]["title"] == "General Election Workflow"
...
def test_rate_limiting():
    ...
    response = client.post("/api/chat", json={"session_id": "ratelimit_final", "message": "hi"})
